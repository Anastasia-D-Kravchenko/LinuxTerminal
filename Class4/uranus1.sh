...
haversine() {
  ...
  # Haversine formula - improved accuracy for small differences
  local a=$(echo "scale=100; sin($dlat / 2)^2 + cos($lat1_rad) * cos($lat2_rad) * sin($dlon / 2)^2" | bc -l)
  local c=$(echo "scale=100; 2 * atan2(sqrt($a), sqrt(1 - $a))" | bc -l)
  local d=$(echo "scale=100; $R * $c" | bc)
